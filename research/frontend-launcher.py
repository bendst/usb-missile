...
class LaunchControl(Tkinter.Frame):
    ...
    def __init__(self):
        ...
        super(LaunchControl, self).__init__()
        self.pack()
        self.master.title("Launch Control")
        self.master.geometry("300x20")

        self.message1 = Tkinter.StringVar()
        self.line1 = Tkinter.Label(self, textvariable=self.message1)
        self.message1.set("Aim (arrow keys) & Fire (return)!")
        self.line1.pack()

        self.master.bind("<KeyPress-" + self.KEY_TURN_UP +
                         ">", self.turnKeyPressed)
        self.master.bind("<KeyRelease-" + self.KEY_TURN_UP +
                         ">", self.turnKeyReleased)
        self.master.bind("<KeyPress-" + self.KEY_TURN_DOWN +
                         ">", self.turnKeyPressed)
        self.master.bind("<KeyRelease-" + self.KEY_TURN_DOWN +
                         ">", self.turnKeyReleased)
        self.master.bind("<KeyPress-" + self.KEY_TURN_LEFT +
                         ">", self.turnKeyPressed)
        self.master.bind("<KeyRelease-" + self.KEY_TURN_LEFT +
                         ">", self.turnKeyReleased)
        self.master.bind("<KeyPress-" + self.KEY_TURN_RIGHT +
                         ">", self.turnKeyPressed)
        self.master.bind("<KeyRelease-" + self.KEY_TURN_RIGHT +
                         ">", self.turnKeyReleased)
        self.master.bind("<Key-" + self.KEY_TURN_MIDDLE +
                         ">", self.turretMiddle)
    ...
    def turretTurn(self):
        self.message1.set(self.directionByteToMsgDic.get(
            self.directionByte, "Turn impossible."))
        self.dev.ctrl_transfer(0x21, 0x09, 0, 0,
                               [0x02, self.directionByte, 0x00, 0x00,
                                0x00, 0x00, 0x00, 0x00])
    ...
    def turretMiddle(self, event):
        ...
        self.dev.ctrl_transfer(
            0x21, 0x09, 0, 0, [0x02, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
        time.sleep(5.36)
        self.dev.ctrl_transfer(
        ...
        time.sleep(0.1)
        self.dev.ctrl_transfer(
            0x21, 0x09, 0, 0, [0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
    ...
    def turretLED(self, event):
        ...
        self.dev.ctrl_transfer(
            0x21, 0x09, 0, 0, [0x03, self.LEDByte, 0x00,
    ...
    def close(self, event):
        ...
        if self.directionByte != 0x00:
            self.dev.ctrl_transfer(
                0x21, 0x09, 0, 0, [0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                   0x00])
        # Turn off LED
        if self.LEDByte != 0x00:
            self.dev.ctrl_transfer(
                0x21, 0x09, 0, 0, [0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                   0x00])
        sys.exit()
